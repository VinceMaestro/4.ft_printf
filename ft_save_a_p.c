...
/*   Updated: 2017/10/24 16:27:36 by vpetit           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			save_it1(t_a_lst *a_lst, va_list *ap, t_ull max)
{
	char		*tmp_str;
	t_u_nbr		nbr;
...
static void			save_it2(t_a_lst *a_lst, va_list *ap, t_ull max)
{
	char		*tmp_str;
	t_u_nbr		nbr;
...
void				ft_save_a_p(t_a_lst *a_lst, t_f_id *f_id, va_list *ap)
{
	...
	if (a_lst && f_id)
	{
		...
		if (max == UINT_MAX || max == ULONG_MAX)
			save_it1(a_lst, ap, max);
		else if (max == USHRT_MAX || max == UCHAR_MAX)
			save_it2(a_lst, ap, max);
	}
}
