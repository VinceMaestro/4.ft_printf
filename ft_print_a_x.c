...
/*   Updated: 2017/10/20 18:42:54 by vpetit           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			ft_updt_p_info(t_p_inf *infos, t_a_lst *a_lst, t_f_id *f_id)
{
	if (infos && a_lst && f_id)
	{
		infos->len_a = ((a_lst->a.s && a_lst->a.s[0] != '0') || (!f_id->w_min && \
			!f_id->prec.period)) ? ft_strlen(a_lst->a.s) : 0;
...
static void		ft_p_left(t_p_inf *infos, t_a_lst *a_lst, t_f_id *f_id)
{
	...
	if ((a_lst->a.s && a_lst->a.s[0] != '0') || (!f_id->w_min && !f_id->prec.period))
		ft_putstr(a_lst->a.s);
